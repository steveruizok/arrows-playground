...
export function getEdgeResizer(boxes: Types.Box[], edge: number): BoxResizer {
  ...
  return throttle(function edgeResize({ x, y }: Types.Point) {
    if (edge === 0 || edge === 2) {
      edge === 0 ? (y0 = y) : (y1 = y)
      my = y0 < y1 ? y0 : y1
      mh = Math.abs(y1 - y0)
...
export function getCornerResizer(
  ...
  return throttle(function cornerResizer({ x, y }: Types.Point) {
    corner < 2 ? (y0 = y) : (y1 = y)
    my = y0 < y1 ? y0 : y1
    mh = Math.abs(y1 - y0)
...
export type BoxResizer = DebouncedFunc<
  (point: Types.Point) => [Types.Box[], Types.Bounds]
>
