 ...
 * @param center When true, will offset the angle by half a sector.
 */
export function getSector(a: number, s = 8, center = true) {
	const offset = PI2 / s / 2
	a = a - PI2 * Math.floor((a + PI) / PI2) + PI2
	const norm = ((a + offset) % PI2) / PI2
	return Math.floor(norm * s)
}
...
/**
 * Check whether a rectangles entirely overlaps another.
 * @param x0 The x-axis coordinate of the first rectangle.
 * @param y0 The y-axis coordinate of the first rectangle.
 * @param w0 The width of the first rectangle.
 * @param h0 The height of the first rectangle.
 ...
 * @param h1 The height of the second rectangle.
 */
export function doRectanglesContain(
	x0: number,
	y0: number,
	w0: number,
	h0: number,
...
export function getSegmentIndexFromIntersectionPoint(
	x: number,
	y: number,
	w: number,
	h: number,
...
export function getRectangleSegmentIntersectedByRay(
	...
	const segments = getRectangleSegments(x, y, w, h)
	const index = segments.findIndex(([sx0, sy0, sx1, sy1]) =>
